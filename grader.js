...
var consoleOutJson = function(checkJson) {
...
var checkHtml = function(htmlfile, url, checksfile) {
	if (htmlfile) {
		var checkJson = checkHtmlString(fs.readFileSync(htmlfile), checksfile);
		consoleOutJson(checkJson);
	} else if (url) {
...
if(require.main == module) {
	...
	if(program.file && !assertFileExists(program.file)) {
		console.log("'-f, --file <html_file>' is not exists");
		process.exit(1);
	}
